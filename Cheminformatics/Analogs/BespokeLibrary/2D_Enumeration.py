...
#######################################
# Remove the ZINCXXYY... from the ZincIDs to save space
...
def Build_Products_and_Outputs(BB_combo):
	...
	for product in All_Products:
		Product_code = "%s__%s__%s_%s" % (Lib_ID, product.purch, product.code_descriptor, product.BB_component_IDs()) # to be in the final form of ID__purch__rxnDescriptor__bb1_bb2_bbn
...
def Determine_and_Run_Reactions(BB_combo):
	...
	for reaction in rxn_tag_to_rxnClass_dict[BB_combo_rxn_tag]:
		...
		except IndexError:	# if Rxn fails, move onto next rxn. If all rxns fail for this BB set, products_list will be empty
			continue
...
#######################################
def product_HACs(BB_files_list):

	max_HAC = int(Bespoke_Lib_Map["Product_Restrictions_Map"]["Max_Product_HAC"])

	disqualification = False

	prod_HAC = 0
	for file_path in BB_files_list:
		file_name = file_path.split("/")[-1]
		BB_file_HAC = int(file_name.split("_")[4][3:])
		prod_HAC += BB_file_HAC
...
# Check if sum BB inputs will be > max product HAC
prod_HAC_disqualification = product_HACs(Reagent_input_files)
if prod_HAC_disqualification:
	exit()
